...
```{r define}
library(MASS) # Required for loading Boston dataset

N <- 70
values <- rep(100, N)

fun <- function(x) kmeans(Boston, 4, nstart = x)
```
...
library(ggplot2)
ggplot(times_df, aes(x = id, y = time)) + theme_bw() + geom_bar(stat = "identity")
```
